"""
Haal ziekenhuizen op uit OpenStreetMap via Overpass.
...
UITGESLOTEN_OSM_ELEMENTEN = {
    ("way", 991333180),  # Frisius Medisch Centrum Harlingen
}
    ...
    for ophalen_url in overpass_urls:
        request = Request(
            ...
            headers={
                ...
                "User-Agent": "DUS-ziekenhuis-fetch/1.0",
            },
            method="POST",
        )
...
def bepaal_match_type(tags):
    ...
    if amenity == "hospital" and healthcare == "hospital":
        return "amenity_and_healthcare_hospital"
...
def element_naar_feature(element):
    ...
    if sleutel in UITGESLOTEN_OSM_ELEMENTEN:
        return None
...
def schrijf_geojson(geojson, output_pad):
    ...
    print(f"Ziekenhuizen opgehaald: {len(geojson['features'])}")
