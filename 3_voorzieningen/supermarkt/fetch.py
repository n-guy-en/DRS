"""
Haal supermarkten op uit OpenStreetMap via Overpass.
...
KETENS_REGEX = "|".join(GROTE_SUPERMARKT_KETENS)
...
(
  ...
  nwr["brand"~"{KETENS_REGEX}", i](area.searchArea);
  nwr["name"~"{KETENS_REGEX}", i](area.searchArea);
);
    ...
    for ophalen_url in overpass_urls:
        request = Request(
            ...
            headers={
                ...
                "User-Agent": "DUS-supermarkt-fetch/1.0",
            },
            method="POST",
        )
...
def bepaal_match_type(tags):
    ...
    for keten in GROTE_SUPERMARKT_KETENS:
        if keten.lower() in keten_tekst:
            return "grote_supermarktketen_name_or_brand"
...
def schrijf_geojson(geojson, output_pad):
    ...
    print(f"Supermarkten opgehaald: {len(geojson['features'])}")
