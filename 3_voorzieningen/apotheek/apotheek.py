"""
Valideer OSM-apotheken met BAG-panden.
...
# %% Stap 2: apotheken en BAG-panden lezen
def lees_apotheken():
    input_pad = (
        BASE_DIR
        / "3_voorzieningen"
    ...
    if not input_pad.exists():
        raise FileNotFoundError(f"Apotheekbestand niet gevonden: {input_pad}")

    print(f"Lees apotheken: {input_pad}")
    apotheken = gpd.read_file(input_pad)

    if apotheken.crs is None:
        apotheken = apotheken.set_crs(CRS_WGS84)

    apotheken = apotheken.to_crs(CRS_RD)
    apotheken = apotheken[apotheken.geometry.notna()].copy()
    apotheken = apotheken[~apotheken.geometry.is_empty].copy()

    apotheken["apotheek_id"] = range(1, len(apotheken) + 1)
    apotheken["apotheek_lon"] = apotheken.to_crs(CRS_WGS84).geometry.x.round(8)
    apotheken["apotheek_lat"] = apotheken.to_crs(CRS_WGS84).geometry.y.round(8)
...
# %% Stap 3: apotheken aan BAG-panden koppelen
def koppel_binnen_pand(apotheken, panden):
    print("Koppel apotheken die binnen een BAG-pand vallen")
...
def koppel_nearest(apotheken_gekoppeld, panden, max_afstand_meter):
    zonder_match = apotheken_gekoppeld[
        apotheken_gekoppeld["pand_id"].isna()
    ].copy()

    if zonder_match.empty:
        return apotheken_gekoppeld

    print(f"Koppel overige apotheken aan nearest BAG-pand binnen {max_afstand_meter} m")
    ...
    met_match = apotheken_gekoppeld[
        apotheken_gekoppeld["pand_id"].notna()
    ].copy()

    kolommen = apotheken_gekoppeld.columns.union(nearest.columns)
    resultaat = pd.concat(
        [
...
def voeg_status_toe(apotheken):
    apotheken["bag_gevalideerd"] = apotheken["pand_id"].notna()
    apotheken["bag_afstand_meter"] = pd.to_numeric(
        apotheken["bag_afstand_meter"],
        errors="coerce",
    ).round(2)
...
def schrijf_output(apotheken):
    ...
    output_csv = BASE_DIR / "3_voorzieningen" / "processed" / "apotheek"
    output_layers.mkdir(parents=True, exist_ok=True)
    output_csv.mkdir(parents=True, exist_ok=True)

    gpkg_pad = output_layers / "apotheek.gpkg"
    csv_pad = output_csv / "apotheek.csv"

    apotheken_wgs84 = apotheken.to_crs(CRS_WGS84)
    apotheken_wgs84.to_file(
        gpkg_pad,
        layer="apotheek",
        driver="GPKG",
    )
    apotheken_wgs84.drop(columns="geometry").to_csv(csv_pad, index=False)
