"""
Download huisartsen uit OpenStreetMap via Overpass.
...
(
  nw["amenity"="doctors"](area.searchArea);
  nw["healthcare"="doctor"](area.searchArea);
);
...
def bepaal_match_type(tags):
    ...
    if amenity == "doctors":
        return "amenity_doctors"

    if healthcare == "doctor":
        return "healthcare_doctor"
...
def schrijf_geojson(geojson, output_pad):
    ...
    print(f"Huisartsen gedownload: {len(geojson['features'])}")
